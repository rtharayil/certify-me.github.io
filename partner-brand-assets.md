---
title: "CertifyMe Partner Brand Assets | Official Logo Downloads"
description: "Download official CertifyMe logos and brand assets for approved partner, institution, customer, media, and co-branding use."
layout: V4LayoutContent
permalink: /partner-brand-assets/
sitemap.priority: 0.4
partner_brand_assets: true
---

<div class="pba-page">
  <section class="pba-hero">
    <div class="container">
      <div class="pba-hero-grid">
        <div class="pba-hero-copy">
          <p class="pba-kicker">CertifyMe partner brand assets</p>
          <h1>CertifyMe Logo &amp; Brand Assets</h1>
          <p class="pba-tagline">The Most Trusted Digital Credential Platform</p>
          <p class="pba-lede">Official CertifyMe brand assets for partners, customers, institutions, and media.</p>
          <a class="pba-button pba-button--primary" href="#pba-logo-library">Download Official Assets <span aria-hidden="true">↓</span></a>
          <div class="pba-trust-line"><span aria-hidden="true">◆</span> Official <span aria-hidden="true">·</span> Trusted <span aria-hidden="true">·</span> Ready to use</div>
        </div>
        <div class="pba-hero-mark" aria-label="CertifyMe primary logo">
          <div class="pba-logo-stage pba-logo-stage--hero">
            <img src="/assets4/images/Logo/1.png" alt="CertifyMe logo" width="7094" height="1940">
          </div>
          <p>Official artwork · 7094 × 1940 px · Transparent PNG</p>
        </div>
      </div>
    </div>
  </section>

  <section id="pba-logo-library" class="pba-section pba-section--library">
    <div class="container">
      <div class="pba-quality-strip" aria-label="Asset quality specifications">
        <div><span>Raster master</span><strong>7094 × 1940 px</strong><small>13.8 megapixels</small></div>
        <div><span>Vector master</span><strong>SVG</strong><small>Resolution-independent</small></div>
        <div><span>Background</span><strong>Transparent</strong><small>Production-ready PNG</small></div>
        <div><span>Colour profile</span><strong>sRGB</strong><small>Web and presentation ready</small></div>
      </div>
      <div class="pba-section-heading">
        <p class="pba-kicker">01 / Core artwork</p>
        <h2>CertifyMe Logo</h2>
        <p>Use the appropriate approved artwork for the background and context. Do not redraw, recolour, or modify the logo.</p>
      </div>

      <div class="pba-feature-grid">
        <article class="pba-feature-card">
          <div class="pba-card-topline"><span>01</span><span>Light background</span></div>
          <div class="pba-logo-stage pba-logo-stage--light">
            <img src="/assets4/images/Logo/1.png" alt="CertifyMe primary logo on a light background" width="7094" height="1940" loading="lazy">
          </div>
          <div class="pba-card-footer">
            <div><h3>Primary Logo</h3><p>PNG · 7094 × 1940 px · Transparent</p></div>
            <a class="pba-download" href="/assets4/images/Logo/1.png" download="certifyme-primary-logo-7094x1940.png" data-brand-download>Download high-res PNG <span aria-hidden="true">↗</span></a>
          </div>
        </article>

        <article class="pba-feature-card pba-feature-card--dark">
          <div class="pba-card-topline"><span>02</span><span>Dark background</span></div>
          <div class="pba-logo-stage pba-logo-stage--dark">
            <img src="/assets4/images/Logo/Logo (10).png" alt="CertifyMe official all-white logo on a dark background" width="7094" height="1940" loading="lazy">
          </div>
          <div class="pba-card-footer">
            <div><h3>All-White Logo</h3><p>PNG · 7094 × 1940 px · Transparent</p></div>
            <a class="pba-download pba-download--light" href="/assets4/images/Logo/Logo%20(10).png" download="certifyme-all-white-logo-7094x1940.png" data-brand-download>Download high-res PNG <span aria-hidden="true">↗</span></a>
          </div>
        </article>
      </div>
    </div>
  </section>

  <section class="pba-section pba-section--formats">
    <div class="container">
      <div class="pba-section-heading">
        <p class="pba-kicker">02 / File library</p>
        <h2>Available Formats</h2>
        <p>Choose vector artwork for print and scale, or transparent raster artwork for digital placements.</p>
      </div>
      <div class="pba-format-grid">
        <a class="pba-format-card" href="/assets4/images/Logo/1.png" download="certifyme-primary-logo-7094x1940.png" data-brand-download>
          <span class="pba-format-icon">PNG</span><strong>Ultra High-Resolution PNG</strong><span>7094 × 1940 px · 13.8 MP · Transparent <b>Download PNG ↗</b></span>
        </a>
        <a class="pba-format-card" href="/assets4/images/Logo/Logo%20(10).png" download="certifyme-all-white-logo-7094x1940.png" data-brand-download>
          <span class="pba-format-icon pba-format-icon--dark">◐</span><strong>All-White Logo</strong><span>7094 × 1940 px · Fully white artwork for dark surfaces <b>Download PNG ↗</b></span>
        </a>
        <a class="pba-format-card" href="/assets4/images/Logo/11.png" download="certifyme-logo-mark.png" data-brand-download>
          <span class="pba-format-icon pba-format-icon--mark">◈</span><strong>Logo Mark</strong><span>500 × 500 px · Transparent standalone symbol <b>Download PNG ↗</b></span>
        </a>
        <div class="pba-format-card pba-format-card--note">
          <span class="pba-format-icon">SOURCE</span><strong>Verified Current Artwork</strong><span>Only current PNG artwork is published here. A current SVG or CDR master will be added once supplied and verified.</span>
        </div>
      </div>
    </div>
  </section>

  <section class="pba-section pba-section--variations">
    <div class="container">
      <div class="pba-section-heading pba-section-heading--split">
        <div><p class="pba-kicker">03 / Approved artwork</p><h2>Logo Variations</h2></div>
        <p>Use only the official files below. Every transparent asset should remain undistorted and maintain its original proportions.</p>
      </div>
      <div class="pba-variation-grid">
        <a class="pba-variation-card" href="/assets4/images/Logo/1.png" download="certifyme-primary-logo-7094x1940.png" data-brand-download><span>Primary Logo</span><div><img src="/assets4/images/Logo/1.png" alt="" loading="lazy"></div><b>7094 × 1940 PNG ↗</b></a>
        <a class="pba-variation-card pba-variation-card--dark" href="/assets4/images/Logo/Logo%20(10).png" download="certifyme-all-white-logo-7094x1940.png" data-brand-download><span>All-White Logo</span><div><img src="/assets4/images/Logo/Logo (10).png" alt="" loading="lazy"></div><b>7094 × 1940 PNG ↗</b></a>
        <a class="pba-variation-card" href="/assets4/images/Logo/11.png" download="certifyme-logo-mark-500x500.png" data-brand-download><span>Logo Mark</span><div><img class="pba-mark-image" src="/assets4/images/Logo/11.png" alt="" loading="lazy"></div><b>500 × 500 PNG ↗</b></a>
        <div class="pba-variation-card pba-variation-card--note"><span>Current file set</span><div class="pba-variation-note">Verified current PNG artwork only.<br><strong>Vector source pending</strong></div><b>Do not use legacy logos</b></div>
      </div>
    </div>
  </section>

  <section class="pba-section pba-section--usage">
    <div class="container">
      <div class="pba-usage-grid">
        <div class="pba-section-heading">
          <p class="pba-kicker">04 / Usage</p>
          <h2>Using the CertifyMe Brand</h2>
          <p>Partners may use these official assets to identify CertifyMe as their digital credentialing platform, technology provider, or partner.</p>
        </div>
        <div class="pba-guidance">
          <p class="pba-guidance-title">Please use the official artwork provided and do not:</p>
          <ul>
            <li>Stretch, distort, or change the logo proportions</li>
            <li>Modify the logo typography or colours</li>
            <li>Add effects, shadows, or outlines</li>
            <li>Recreate the logo manually</li>
          </ul>
        </div>
      </div>
    </div>
  </section>

  <section class="pba-section pba-section--recognition">
    <div class="container">
      <div class="pba-recognition-card">
        <div>
          <p class="pba-kicker">05 / Partner recognition</p>
          <h2>Proudly Powered by CertifyMe</h2>
          <p>A simple, professional co-branding arrangement for university websites, partner announcements, presentations, certificates, events, and marketing materials.</p>
        </div>
        <div class="pba-cobrand" aria-label="Partner logo and CertifyMe logo co-branding example">
          <div class="pba-partner-placeholder"><span>Partner Logo</span></div>
          <span class="pba-plus" aria-hidden="true">+</span>
          <div class="pba-cobrand-logo"><img src="/assets4/images/Logo/1.png" alt="CertifyMe logo" loading="lazy"></div>
        </div>
      </div>
    </div>
  </section>

  <footer class="pba-footer">
    <div class="container">
      <div class="pba-footer-inner">
        <div class="pba-footer-brand">
          <img src="/assets4/images/Logo/1.png" alt="CertifyMe logo" loading="lazy">
          <p>The Most Trusted Digital Credential Platform</p>
          <span>Official Partner Brand Assets</span>
        </div>
        <button class="pba-button pba-button--outline" type="button" data-download-all>Download All Assets <span aria-hidden="true">↓</span></button>
      </div>
    </div>
  </footer>
</div>

<script>
document.addEventListener('DOMContentLoaded', function () {
  var button = document.querySelector('[data-download-all]');
  if (!button) return;
  button.addEventListener('click', function () {
    var downloads = document.querySelectorAll('[data-brand-download]');
    var uniqueDownloads = Array.from(downloads).filter(function (link, index, links) {
      return links.findIndex(function (candidate) { return candidate.href === link.href; }) === index;
    });
    uniqueDownloads.forEach(function (link, index) {
      window.setTimeout(function () { link.click(); }, index * 180);
    });
  });
});
</script>